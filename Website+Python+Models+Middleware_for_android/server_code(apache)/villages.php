...
$sql = "SELECT * from mytable WHERE taluka='$v' AND lat IS NOT NULL;; ";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $lat[$i]=$row['lat'];
        $lon[$i]=$row['lon'];
        $village[$i]=$row['village'];
        $i++;

        }
    }
$sql = "SELECT * from taluka WHERE id='$v';";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
...
$sql="SELECT * from rates WHERE (commodity, date,market) IN (SELECT commodity,max(date),market from rates where market='$v' group by commodity);";
$result = $conn->query($sql);
    ...
    <header> 
        <div class="container">
            <div class="intro-text">
                <div class="intro-lead-in"><font color="black"><?php echo $name;?></font></div>
                <p> <font size="6" color="black"><?php echo $desc;?></font></p>
...
<script>
    ...
    var mymap = L.map('mapid').setView(<?php echo htmlspecialchars($location);?>, 10);
    ...
    L.marker(<?php echo htmlspecialchars($str);?>).addTo(mymap).bindPopup(<?php echo json_encode($print);?>+"<br><form action='info.php' method='post'><input type='hidden'  value='"+<?php echo json_encode($taluka);?>+"' name='taluka'><input type='hidden'  value='"+<?php echo json_encode($print);?>+"' name='village'><input type='submit' class='page-scroll btn btn-xl' style='border-radius: 3px;font-size: 10px; padding: 4px 6px ; '  value='Know about "+<?php echo json_encode($print);?>+"'></form>").openPopup();
